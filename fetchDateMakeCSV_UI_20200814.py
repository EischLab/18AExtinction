...
def SelectDatesByTrialNumber(InDB,n):
  ...
  cur.execute('SELECT DISTINCT ID,Date FROM TSData ORDER BY date(Date) DESC')#if ASC, then ordered by time; if DESC, then ordered by Animal ID - I am not sure why yet; I wrote DESC for a prev test and it was useful
